...
def process_video(cap):
    ...
    while True:
        ...
        # If the model detects a coffee stain, draw a blue outline and calculate the percentage
        if prediction == 1:
            resin_detected = True
            end_time = time.time()

            # Calculate percentage of stained area within the ROI
            stained_pixels = np.count_nonzero(model.predict([cv2.mean(roi_area[y:y+h, x:x+w])[0]]))
            total_pixels = roi_area.size // 3  # 3 channels in BGR
            stain_percentage = (stained_pixels / total_pixels) * 100

            # Draw a blue outline around the detected coffee-stained area
            cv2.rectangle(frame, (x, y), (x + w, y + h), (255, 0, 0), 2)
            
            # Show the stain percentage on the frame
            cv2.putText(frame, f"Stain Area: {stain_percentage:.2f}%", (w - 150, h - 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
